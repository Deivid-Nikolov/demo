...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($name_err) && empty($address_err) && empty($citizen_num_err) && empty($middle_err) && empty($last_err) && empty($family_err) && empty($email_err) && empty($number_err) && empty($yearly_err)){
        ...
        $sql = "UPDATE registered SET first_name=?, middle_name=?, last_name=?, egn=?, email=?, number=?, address=?, yearly_income=?, has_family = ? WHERE id=?";
    ...
    <div class="wrapper">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    <?php
                        $link = mysqli_connect('localhost','root','','plamen_kursova')or die(mysqli_error());
                        $p_id = trim($_GET["id"]);
                        $user_query=mysqli_query($link,"select * from registered where id='$p_id'")or die(mysqli_error());
